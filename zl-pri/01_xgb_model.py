...
target = 'is_stage6_due10_new'
dev_sample = model_data.query(f"apply_month <= '2024-04' and mob_i>=1 and {target}==[0, 1]").reset_index(drop=True)
oot_sample = model_data.query(f"apply_month >= '2024-05' and mob_i>=1 and {target}==[0, 1]").reset_index(drop=True)
...
target = 'is_stage6_due10_new'
save_model="../model/bcard_lgbm_mobgt1"
n_folds=5
seed=42
use_stratified=True
...
model_data['bcard_card_score_v1_0'] = model_data[score_variables].apply(lambda x: round(x.sum(), 2), axis=1)


model_helper.model_group_monitor(
    model_data.query(f"tx_date>='2024-05-01' and {target}==[0, 1]"),
    target,
    "bcard_card_score_v1_0",
    higher_better=True,
    number_of_groups=10,
)
...
gbm_model = LGBMClassifier(**{'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.95, 'importance_type': 'gain', 'learning_rate': 0.05, 'max_depth': 3, 'min_child_samples': 250, 'min_child_weight': 5, 'min_split_gain': 0.0, 'n_estimators': 300, 'n_jobs': -1, 'num_leaves': 30, 'objective': None, 'random_state': None, 'reg_alpha': 5, 'reg_lambda': 2, 'silent': 'warn', 'subsample': 0.95, 'subsample_for_bin': 1000, 'subsample_freq': 0, 'sample_weight': None, 'seed': 42}
)
...
shap_imp_1 = calculate_importance(gbm_model, dev_sample, input_variables, importance_type='shap')
shap_imp_1.head(10)
sel_cols = shap_imp_1['Feature'].tolist()[:100]


gbm_model = LGBMClassifier(**{'boosting_type': 'gbdt', 'class_weight': None, 'colsample_bytree': 0.95, 'importance_type': 'gain', 'learning_rate': 0.05, 'max_depth': 3, 'min_child_samples': 220, 
...
model_data['bcard_xgb_score'] = model_data['bcard_xgb_proba'].apply(score_card)

model_data.query(f"{target}==[0, 1] and apply_month>='2024-05'").groupby(['cnt']).apply(lambda x: f_evalutor_3(x, "bcard_xgb_score", target, "score"))


model_data.query(f"{target}==[0, 1] and apply_month>='2024-05 and mob_i>=3'").groupby(['cnt']).apply(lambda x: f_evalutor_3(x, "bcard_xgb_score", target, "score"))
